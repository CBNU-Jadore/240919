...
def getProduct(request, pk):
    try:
        product = Product.objects.get(_id=pk)   # Product(장고모델)의 _id 필드가 pk 값(URL에서 전달된 값)과 일치하는 객체를 조회
    except Product.DoesNotExist:
        raise NotFound(detail="Product not found", code=404)
    serializer = ProductSerializer(product, many=False) # Product 모델의 데이터를 직렬화(장고모델 -> JSON)
